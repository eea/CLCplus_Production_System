...
from src.v1.models.model_gaf_services import RequestSampleExtractionTesting
...
router_gaf_sample_extraction_testing = APIRouter()
...
@router_gaf_sample_extraction_testing.post('/gaf-sample-extraction-testing',
                                           response_model=ResponseServices,
                                           status_code=201,
                                           summary='POST definition for triggering the sample extraction testing '
                                                   'workflow for GAF',
                                           description="""
...
""")
def gaf_sample_extraction_testing(payload: RequestSampleExtractionTesting,
                                  user_info=Security(get_auth, scopes=["admin gaf"])):
    service_dag_name = 'sample_extraction_dag_testing'
    ...
    try:
        ...
        logger.error(f'sample_extraction_testing: {err}\n{traceback.format_exc()}')
