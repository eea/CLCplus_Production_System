...
from src.v1.models.model_gaf_services import RequestApplyModel
...
router_gaf_apply_model = APIRouter()
...
@router_gaf_apply_model.post('/gaf-apply-model',
                             response_model=ResponseServices,
                             status_code=201,
                             summary='POST definition for triggering the apply model workflow for GAF',
                             description="""
...
""")
def gaf_apply_model(payload: RequestApplyModel,
                    user_info=Security(get_auth, scopes=["admin gaf"])):

    service_dag_name = 'apply_model_dag'
    ...
    try:
        ...
        logger.error(f'apply_model: {err}\n{traceback.format_exc()}')
