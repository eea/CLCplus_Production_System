...
from src.v1.models.model_gaf_services import RequestPrepareSamples
...
router_gaf_prepare_samples = APIRouter()
...
@router_gaf_prepare_samples.post('/gaf-prepare-samples',
                                 response_model=ResponseServices,
                                 status_code=201,
                                 summary='POST definition for triggering the prepare samples workflow for GAF',
                                 description="""
...
""")
def gaf_prepare_samples(payload: RequestPrepareSamples,
                        user_info=Security(get_auth, scopes=["admin gaf"])):
    service_dag_name = 'prepare_samples_dag'
    ...
    try:
        ...
        logger.error(f'prepare_samples: {err}\n{traceback.format_exc()}')
