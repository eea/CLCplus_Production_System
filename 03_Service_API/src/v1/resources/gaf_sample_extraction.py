...
from src.v1.models.model_gaf_services import RequestSampleExtraction
...
router_gaf_sample_extraction = APIRouter()
...
@router_gaf_sample_extraction.post('/gaf-sample-extraction',
                                   response_model=ResponseServices,
                                   status_code=201,
                                   summary='POST definition for triggering the sample extraction workflow for GAF',
                                   description="""
...
""")
def gaf_sample_extraction(payload: RequestSampleExtraction,
                          user_info=Security(get_auth, scopes=["admin gaf"])):
    service_dag_name = 'sample_extraction_dag'
    ...
    try:
        ...
        logger.error(f'sample_extraction: {err}\n{traceback.format_exc()}')
