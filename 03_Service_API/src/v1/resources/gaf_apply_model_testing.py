...
from src.v1.models.model_gaf_services import RequestApplyModelTesting
...
router_gaf_apply_model_testing = APIRouter()
...
@router_gaf_apply_model_testing.post('/gaf-apply-model-testing',
                                     response_model=ResponseServices,
                                     status_code=201,
                                     summary='POST definition for triggering the apply model testing workflow for GAF',
                                     description="""
...
""")
def gaf_apply_model_testing(payload: RequestApplyModelTesting,
                            user_info=Security(get_auth, scopes=["admin gaf"])):
    service_dag_name = 'apply_model_dag_testing'
    ...
    try:
        ...
        logger.error(f'apply_model_testing: {err}\n{traceback.format_exc()}')
